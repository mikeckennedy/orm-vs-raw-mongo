...
def generate_charts(results: list[BenchmarkResult], output_dir: str = 'output'):
    ...
    reads = {k: v for k, v in by_name.items() if any(r.benchmark.op_type == OpType.READ for r in v.values())}
    writes = {k: v for k, v in by_name.items() if any(r.benchmark.op_type == OpType.WRITE for r in v.values())}
...
def _overhead_chart(
    ...
    ax.bar(positions_b, beanie_overheads, bar_width, label='Beanie', color=COLORS['beanie'])
    ax.bar(
        positions_m,
        me_overheads,
        bar_width,
